...
pub fn refresh(
    ...
    if (document.mode == .COMMAND) {
        ...
        screen_x = lineNumberWidth(document) + document.render_x - document.col_offset;
...
fn drawRows(
    ...
    const gutter_width: u16 = @intCast(lineNumberWidth(document));

    const gutter_window = window.child(.{
        .x_off = 0,
        .width = gutter_width,
        .height = text_height,
    });
    const text_window = window.child(.{
        ...
        .width = window.width -| gutter_width,
        .height = text_height,
    });
    ...
    for (0..text_height) |screen_row| {
        ...
        if (file_row < document.rows.items.len) {
            ...
            var line_number: []const u8 = undefined;
            switch (document.config.line_numbers) {
                .relative => {
                ...
                std.mem.startsWith(u8, row.chars.items[document.cursor_x..], query.?))
            {
                ...
                const visible_end = @min(match_end_rx, document.col_offset + text_window.width);
...
fn lineNumberWidth(document: *const editor.Editor) usize {
    const line_count = @max(document.rows.items.len, 1);
...
fn displayRelativeLineNumber(
    document: *const editor.Editor,
    file_row: usize,
) usize {
    if (file_row == document.cursor_y) {
        return file_row + 1;
    }

    return if (file_row > document.cursor_y)
        file_row - document.cursor_y
    else
        document.cursor_y - file_row;
}
